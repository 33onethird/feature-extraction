...
public class SmaliAnalyzer {
	...
	public final static Pattern urlPattern = Pattern.compile("const-string .+(http:\\/\\/.+)\""); // Regex patterns for
																									// URLs
	...
	public final static Pattern cipherPattern = Pattern.compile("const-string.*\\\"(.*)\\\""); // Regex for details on
																								// Cipher
	...
	/**
	 * Uses the prebuilt mapping and list to initialize the analysis and selects the
	 * real permissions used by looking up every extracted api call in the map
	 * MethodCalls <-> Permission(s)
	 *
	 * @param smali
	 *            Folder with decompiled code
	 * @param prebuiltPermissionMap
	 *            Datastructure mapping MethodCalls <-> Permission(s)
	 * @param prebuiltSuspCallTemplate
	 *            list of suspicious calls
	 */
		...
		try {
			...
			for (String call : apiCalls) {
				...
				if (permissionMap.containsKey(call)) {
					for (String permission : splitToList(permissionMap.get(call))) { // if a call requires more than one
																						// permission, split the string
																						// to an iterable list
	...
	/**
	 * Recursive method that analyzes all files in its directory and subdirectories
	 * and stores its findings in sets
	 *
	 * @param directoryName
	 *            initial directory
					...
					if (!lineFromFile.equals("")) { // Don't bother with empty lines
						...
						for (String suspCall : suspCallSet) {
							if (lineFromFile.contains(suspCall)) { // If the current line contains a suspicious api call
								if (lineFromFile.contains("Cipher")) { // If it is a cipher, look at the previous line
																		// to get more information e.b.
