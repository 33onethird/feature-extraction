...
public class ManifestAnalyzer {
	...
	public ManifestAnalyzer(File manifest) {
       ...
       for (int temp = 0; temp < nList.getLength(); temp++) {
           Node nNode = nList.item(temp);
           Element eElement = (Element) nNode;
           //System.out.println(nNode.getNodeName()+" "+eElement.getAttribute(""));
           if(eElement.hasAttribute("android:theme")||(eElement.hasAttribute("android:label")&&eElement.hasAttribute("android:screenOrientation"))) {
           ...
           if(nNode.hasChildNodes()) {
        	   ...
        	   for(int i =0; i<children.getLength();i++) {
        		   if(children.item(i).getNodeName().equals("intent-filter")&&children.item(i).hasChildNodes()) {
        			   NodeList actions= children.item(i).getChildNodes();
        			   	for(int j=0;j<actions.getLength();j++) {
        			   		...
        			   		if(currenNode.getNodeName().equals("action")) {
        	        			  ...
        	        			  if(currentActionElement.getAttribute("android:name").equals("android.intent.action.MAIN")) {
        	        	        	   activity.add("activity::"+getLastString(eElement.getAttribute("android:name")));
        	        			  }
        			   		}
        	 ...
        	 for(int i =0; i<children.getLength();i++) {
                if(!children.item(i).getNodeName().equals("#text")) {
        	 	...
        	 	if(child.hasAttribute("android:name")) {
        	           ...
        	           System.out.println("Adding: "+child.getAttribute("android:name"));
        	 	}
                }
        	 }
   ...
   for (int temp = 0; temp < nList.getLength(); temp++) {
	...
	public Set<String> getServiceReciever() {
